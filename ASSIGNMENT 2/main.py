...
# Products List (derived from Day 1 and expected Day 2 outputs)
products = [
...
# Orders List (for Bonus tasks)
orders = []

# Customer Feedback List (for Q3)
feedback = []

# ==========================================
# Day 1 Emulated Endpoints (for context)
# ==========================================
@app.post("/orders")
def place_order(order_data: dict):
    new_order = order_data.copy()
    new_order["order_id"] = len(orders) + 1
    # BONUS: Change 'confirmed' to 'pending'
...
def filter_products(
    ...
    min_price: Optional[int] = Query(None, description="Minimum price") # Added for Q1
):
    result = products
    if category:
